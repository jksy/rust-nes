...
impl Cpu {
    ...
    fn adc<T: AddressingMode>(&mut self, addr: T) -> bool {
        ...
        self.set_flag(
            FLAG_OVF,
            (target ^ value) & 0x80 == 0 && (target ^ result) & 0x80 == 0x80,
        );
        self.a = result as u8;
    ...
    fn print_diff(&self, before: Cpu) {
        if self.a != before.a {
            info!("a: {:x} -> {:x}", before.a, self.a)
        }
        if self.x != before.x {
            info!("x: {:x} -> {:x}", before.x, self.x)
        }
        if self.y != before.y {
            info!("y: {:x} -> {:x}", before.y, self.y)
        }
        if self.pc != before.pc {
            info!("pc: {:x} -> {:x}", before.pc, self.pc)
        }
        if self.s != before.s {
            info!("s: {:x} -> {:x}", before.s, self.s)
        }
        if self.p != before.p {
    ...
    fn debug(&mut self) {
        ...
        info!(
            "=====CPU(cycle:[{:07}({}s)],pc:[{:02x}]====",
            self.cycle, time, self.pc
        );
        info!(
        ...
        info!(
            "p[{:x}] CRY:{}, ZER:{}, IRQ:{}, DEC:{}, BRK:{}, RSV:{}, OVF:{}, NEG:{}",
            self.p,
            (self.p & FLAG_CRY) != 0,
            (self.p & FLAG_ZER) != 0,
            (self.p & FLAG_IRQ) != 0,
